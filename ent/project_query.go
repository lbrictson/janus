...
func (_q *ProjectQuery) QueryProjectUsers() *ProjectUserQuery {
...
func (_q *ProjectQuery) QueryHistory() *JobHistoryQuery {
	query := (&JobHistoryClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *ProjectQuery) WithProjectUsers(opts ...func(*ProjectUserQuery)) *ProjectQuery {
	query := (&ProjectUserClient{config: _q.config}).Query()
...
func (_q *ProjectQuery) WithHistory(opts ...func(*JobHistoryQuery)) *ProjectQuery {
	query := (&JobHistoryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProjectQuery) loadProjectUsers(ctx context.Context, query *ProjectUserQuery, nodes []*Project, init func(*Project), assign func(*Project, *ProjectUser)) error {
...
func (_q *ProjectQuery) loadHistory(ctx context.Context, query *JobHistoryQuery, nodes []*Project, init func(*Project), assign func(*Project, *JobHistory)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
