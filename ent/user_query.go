...
func (uq *UserQuery) QueryProjectUsers() *ProjectUserQuery {
	query := (&ProjectUserClient{config: uq.config}).Query()
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withProjectUsers: uq.withProjectUsers.Clone(),
...
func (uq *UserQuery) WithProjectUsers(opts ...func(*ProjectUserQuery)) *UserQuery {
	query := (&ProjectUserClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withProjectUsers = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withProjectUsers; query != nil {
		if err := uq.loadProjectUsers(ctx, query, nodes,
			func(n *User) { n.Edges.ProjectUsers = []*ProjectUser{} },
			func(n *User, e *ProjectUser) { n.Edges.ProjectUsers = append(n.Edges.ProjectUsers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadProjectUsers(ctx context.Context, query *ProjectUserQuery, nodes []*User, init func(*User), assign func(*User, *ProjectUser)) error {
	...
	for _, n := range neighbors {
		fk := n.user_project_users
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_project_users" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_project_users" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
