...
func (uc *UserCreate) SetEncryptedPassword(b []byte) *UserCreate {
	uc.mutation.SetEncryptedPassword(b)
	return uc
}
...
func (uc *UserCreate) AddProjectUsers(p ...*ProjectUser) *UserCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return uc.AddProjectUserIDs(ids...)
}
