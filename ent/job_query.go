...
func (_q *JobQuery) QueryHistory() *JobHistoryQuery {
...
func (_q *JobQuery) QueryVersions() *JobVersionQuery {
	query := (&JobVersionClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *JobQuery) WithHistory(opts ...func(*JobHistoryQuery)) *JobQuery {
	query := (&JobHistoryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *JobQuery) WithVersions(opts ...func(*JobVersionQuery)) *JobQuery {
	query := (&JobVersionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *JobQuery) loadHistory(ctx context.Context, query *JobHistoryQuery, nodes []*Job, init func(*Job), assign func(*Job, *JobHistory)) error {
...
func (_q *JobQuery) loadVersions(ctx context.Context, query *JobVersionQuery, nodes []*Job, init func(*Job), assign func(*Job, *JobVersion)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Job)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
